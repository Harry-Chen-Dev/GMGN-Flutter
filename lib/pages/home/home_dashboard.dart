...
class HomeDashboard extends StatelessWidget {
  ...
  Widget _buildTopTokens(BuildContext context) {
    return Consumer<WalletProvider>(
      builder: (context, walletProvider, child) {
        if (walletProvider.isLoading) {
          return _buildLoadingSection(context, '热门币种');
        }

        final wallet = walletProvider.wallet;
        if (wallet == null || wallet.tokens.isEmpty) {
          return _buildEmptySection(context, '热门币种', '暂无持仓');
        }
    ...
    return Consumer<MarketProvider>(
      builder: (context, marketProvider, child) {
        if (marketProvider.isLoading) {
          return _buildLoadingSection(context, '市场概览');
        }

        final overview = marketProvider.overview;
        if (overview == null) {
          return _buildEmptySection(context, '市场概览', '暂无数据');
        }
